...
import com.impactupgrade.nucleus.util.OAuth2Client;
...
public class VirtuousClient {
  ...
  private static OAuth2Client.Tokens tokens;
  ...
  private HttpClient.HeaderBuilder headers() {
    ...
    if (tokens == null) {
      tokens = new OAuth2Client.Tokens(accessToken, null, refreshToken);
    }
    tokens = OAuth2Client.refreshTokens(tokens, tokenServerUrl);
    if (tokens == null) {
      tokens = OAuth2Client.getTokens(username, password, tokenServerUrl);
    }
...
//      //curl -d "grant_type=password&username=YOUR_EMAIL&password=YOUR_PASSWORD&otp=YOUR_OTP" -X POST https://api.virtuoussoftware.com/Token
    ...
    return HttpClient.HeaderBuilder.builder().authBearerToken(tokens.accessToken());
  }
