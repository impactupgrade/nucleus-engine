...
public class BrevoCommunicationService extends AbstractCommunicationService {
  ...
  protected ExistingContacts getExistingContacts(EnvironmentConfig.CommunicationPlatform config,
    ...
    try {
      List<GetContactDetails> listContacts = brevoClient.getContactsFromList(list.id);
      listContacts.forEach(contact -> existingContacts.emailsToIds.put(contact.getEmail().toLowerCase(Locale.ROOT), contact.getId().toString()));
  ...
  @Override
  protected void executeBatchArchive(Set<String> emails, ExistingContacts existingContacts,
    ...
    brevoClient.removeContactsFromList(emails, list.id);
  }
  ...
  protected Set<String> getUnsubscribedEmails(Calendar lastSync,
      EnvironmentConfig.CommunicationPlatform config, EnvironmentConfig.CommunicationList list) throws Exception {
    BrevoClient brevoClient = env.brevoClient(config);
    List<GetContactDetails> listContacts = brevoClient.getContactsFromList(lastSync, list.id);
    List<GetContactDetails> unsubscribed = listContacts.stream()
        .filter(c -> unsubscribedFromList(c, list.id))
        .toList();
  ...
  protected Set<String> getEmails(List<GetContactDetails> contacts) {
    return contacts.stream().map(c -> c.getEmail().toLowerCase(Locale.ROOT)).collect(Collectors.toSet());
  }
